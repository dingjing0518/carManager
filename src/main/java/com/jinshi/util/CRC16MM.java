...
public class CRC16MM {
    ...
    public byte[] getSendData(int i, int x) {
        byte[] reByte = new byte[8];
        reByte[0] = 0x03;
        reByte[1] = 0x03;

        return CRC16MM.getSendBuf(reByte);
    }
...
//        s.getBufHexStr()
        ...
        byte[] bytesa = {0x00, 0x64, (byte) 0xFF, (byte) 0xFF,0x30,0x0C,0x01, (byte) 0xD4, (byte) 0xC1,0x42,0x31,0x32,0x33,0x34,0x35,0x36, (byte) 0xBE, (byte) 0xAF,0x00,0x00};
        byte[] sendBuf = getSendBuf(bytesa);
        String bufHexStr = getBufHexStr(sendBuf);
        byte[] bytes1 = HexString2Buf(bufHexStr);
        String bufHexStr1 = getBufHexStr(bytes1);
        System.out.println(bufHexStr);
        System.out.println(bufHexStr1);
    }
